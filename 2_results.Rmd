...
### Mean Discharge
```{r, fig.cap= "*Figure 1.* Trends for all 12 months mean discharge data from 1995 to 2021 for the upstream site in the Delaware River."}
...
ggplot(q_month_means, aes(x = year, y = q_mean)) +
  ...
  xlab('Year')
```
On *Figure 1.* we see all of the mean discharge values for the water years from 1995 
...
```{r, fig.cap= "*Figure 2.* August trends for the upstream site in the Delaware River using mean discharge data from 1995 to 2021."}
...
```{r,fig.cap= "*Figure 3.* September trends for the upstream site in the Delaware River using mean discharge data from 1995 to 2021."}
...
print(up_sept_mk)
```
Now Looking at *Figure 3.* for September, there does not seem to be any visible trends 
...
### Median Discharge
```{r,fig.cap= "*Figure 4.* Trends for the upstream site in the Delaware River using median discharge data from 1995 to 2021."}
#Upstream Site
ggplot(q_month_means, aes(x = year, y = q_median)) +
  ...
  xlab('Year')
```
*Figure 4*, is very similar to *Figure 1.* however, the data plots are closer 
...
### Upstream site
```{r, fig.cap= "*Figure 5.* Average specific conductivity trends for the upstream site in the Delaware River using data from 1995 to 2021."}
site_no1 <- "01463500"
...
ggplot(sc_mean_t, aes(x = year, y = sc_mean1)) +
  ...
  xlab('Year')
```
*Figure 5.* shows the mean specific conductivity for the upstream site for the Delaware River, which August and September have the largest values.
...
```{r, fig.cap= "*Figure 6.* Mean Specific Conductivity trends for the Delaware River's Middle site and Estuary site from March 2021 to April 2022."}
...
Looking at *Figure 6.* which shows specific conductivity at the Middle and Estuary sites, we can see that the Estuary has a lot more saltwater which makes sense for it's location. However, the middle site has more dramatic increases in saltwater that occur in July and Janurary. 
...
```{r, fig.cap= "*Figure 7.* Comparison of the Delaware River's upstream site discharge and specific conductivity from 1995 to 2021."}
#Upstream Site
...
```
*Figure 7.* shows how discharge and specific conductivity act with each other. 
